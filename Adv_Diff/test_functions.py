...
def plot_simulations(init_f, n=[6,6,6], T=0.5, c=1, nu=0.1, shots=10**6, d=4):
    ...
    for i in range(3):
        ...
        results.append((x, f0, z_list[0], w_list[0], max_err, comp))

    # Define a common grid (based on the finest resolution
    n_max = max(n)
    xmin = max([x.min() for (x, _, _, _, _, _) in results])
    xmax = min([x.max() for (x, _, _, _, _, _) in results])
    x_common = np.linspace(xmin, xmax, 2**n_max)

    # Interpolate results to common grid
    interp_results = []
    for (x, f0, z, w, max_err, comp) in results:
        f_interp = interp1d(x, f0, kind="linear")
        z_interp = interp1d(x, z, kind="linear")
        w_interp = interp1d(x, w, kind="linear")
        interp_results.append((f_interp(x_common), z_interp(x_common), w_interp(x_common), max_err, comp))

    # Create summary table
    table = [
        [f"Order {orders[i]} (n={n[i]})", max_err[0][0], max_err[0][1], comp[0][0], comp[0][1]]
        for i, (_, _, _, _, max_err, comp) in enumerate(results)]
    print("-- SUMMARY --")
    ...
    axes[0].plot(x_common, interp_results[0][0], lw=1)
    axes[0].set_title(rf'Initial Condition', fontsize=14)

    # Plot all orders together
    for i in range(3):
        f0_interp, z_interp, w_interp, _, _ = interp_results[i]
        if i == 0:
            axes[1].plot(x_common, w_interp, lw=1, label=f'Fourier')
        axes[1].plot(x_common, z_interp, lw=1,
...
plot_simulations(Gaussian, n=[8,7,6], T=1, c=1, nu=0)
